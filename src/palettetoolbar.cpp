...
PaletteToolBar::PaletteToolBar( QWidget *parent )
    : KToolBar( parent )
{
  ...
  m_layout = new QBoxLayout( d, base );
  ...
  m_lblPreview->setWhatsThis( i18n( "Preview\n\nThis is a 1:1 preview"
      " of the current icon" ) );
  ...
  m_currentColorView->setWhatsThis( i18n( "Current color\n\nThis is the currently selected color" ) );
  ...
  m_sysColors->setWhatsThis( i18n( "System colors\n\nHere you can select"
  ...
  m_customColors->setWhatsThis( i18n( "Custom colors\n\nHere you can"
...
{
  // FIXME // if(  barPos() == Floating )
  // FIXME //   o = o == Qt::Vertical ? Qt::Horizontal : Qt::Vertical;

  QBoxLayout::Direction d = o == Qt::Horizontal? QBoxLayout::LeftToRight
                              : QBoxLayout::TopToBottom;
  m_layout->setDirection( d );

  // FIXME //Q3DockWindow::setOrientation( o );
}
...
{
    // FIXME // m_lblPreview->setBackgroundPixmap(pixmap);
}
...
void PaletteToolBar::currentColorChanged(uint color)
{
  ...
  else
  {
    ...
    QRgb squareColour = qRgb(r, g, b);

    p.setPen(Qt::NoPen);
    p.setBrush(QColor(squareColour));
