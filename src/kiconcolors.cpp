...
KDrawColors::KDrawColors(QWidget *parent) : KColorGrid(parent, 3)
{
  kDebug(4640) << "KDrawColors - constructor";
  setCellSize(17);
  setGrid(true);
  setGridState(KColorGrid::Shaded);
  selected = 0;
  kDebug(4640) << "KDrawColors - constructor - done";
}
...
void KDrawColors::paintCell( QPainter *painter, int row, int col )
{
  ...
  qDrawShadePanel( painter, d, d, cellSize()-d, cellSize()-d,
                   palette(), true, 1, &brush);
  if ( row * numCols() + col == selected) {
     // FIXME //painter->drawWinFocusRect( d+1, d+1, cellSize()-(2*d)+1, cellSize()-(2*d)+1 );
  }
}
...
{
  kDebug(4640) << "KSysColors - constructor";
  ...
  kDebug(4640) << "KSysColors - constructor - before setColor";
  for(int i = 0; i < numcells; i++)
  {
    setColor(i, iconpalette[i]|OPAQUE_MASK);
  }
  kDebug(4640) << "KSysColors - constructor - done";
}

KCustomColors::KCustomColors(QWidget *parent) : KDrawColors(parent)
{
  kDebug(4640) << "KCustomColors - constructor";
  setNumRows(3);
  setNumCols(6);
  fill(TRANSPARENT|OPAQUE_MASK);
  setFixedSize(numCols()*cellSize(), numRows()*cellSize());
  ...
  for(int i = 0; i < numRows()*numCols(); i++)
    freecells[i] = true;
  kDebug(4640) << "KCustomColors - constructor - done";
}
...
void KCustomColors::addColor(uint c)
{
  if(!contains(c))
  {
    ...
    if(f != -1)
    {
      ...
      if(!color.isValid())
      {
        kDebug(4640) << "KCustomColors::addColor: Not a valid color: " << c;
        return;
      }
      //kDebug(4640) << "KCustomColors::addColor: Adding color: " << c << " - " << qRed(c) << " " << qGreen(c) << " " << qBlue(c);
      setColor(f, c);
      freecells[f] = false;
    }
  }
