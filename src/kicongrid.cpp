...
KGridView::KGridView(QImage *image, K3CommandHistory* history, QWidget *parent, const char *name)
: QFrame(parent)
{
  _corner = 0L;
  _hruler = _vruler = 0L;
  ...
  viewport = new Q3ScrollView(this);
  ...
  if(props->bgMode() == Qt::FixedPixmap)
  {
    ...
    viewport->viewport()->setPalette ( palette );
    _grid->setPalette( palette );
  }
  else
  {
    palette.setColor( backgroundRole() , props->bgColor( ) );
    viewport->viewport()->setPalette( palette );
  }
  ...
  str = i18n( "Rulers\n\nThis is a visual representation of the current"
      " cursor position" );
  _hruler->setWhatsThis( str );
  _vruler->setWhatsThis( str );
...
void KGridView::setSizes()
{
  if(KIconEditProperties::self()->showRulers())
  {
    ...
    _hruler->setMaximum(_grid->width()+20);
    _vruler->setMaximum(_grid->height()+20);
...
void KGridView::paintDropSite()
{
    ...
    p.begin( _grid );
#ifdef __GNUC__
#warning "qt4 ???? p.setRasterOp (NotROP);"
#endif	
...
{
  kDebug(4640) << "KGridView::resizeEvent";
...
KIconEditGrid::KIconEditGrid(QImage *image, K3CommandHistory* h, QWidget *parent)
 : KColorGrid(parent, 1)
{
    img = image;
    history = h;
    ...
    connect( h, SIGNAL(commandExecuted(K3Command *)), this, SLOT(updatePreviewPixmap() ));
    createCursors();
...
{
    kDebug(4640) << "KIconEditGrid - destructor: done";
}
...
void KIconEditGrid::paintEvent(QPaintEvent *e)
{
  ...
  if(!paintCellsRect.isEmpty())
  {
    ...
    //kDebug(4640) << "Image render elapsed: " << time.elapsed();
    ...
    if(!paintCellsRect.isEmpty())
    {
      ...
      for(int column = firstColumn; column <= lastColumn; column++)
      {
        for(int row = firstRow; row <= lastRow; row++)
        {
          ...
          {
            // FIXME //p->drawWinFocusRect(x + 1, y + 1, cellSize() - 2, cellSize() - 2);
          }
          else
          {
            switch( tool )
...
void KIconEditGrid::mouseMoveEvent( QMouseEvent *e )
{
    ...
    switch( tool )
    {
        ...
        {
            if( !m_command )
                m_command = new K3MacroCommand( i18n("Free Hand") );
...
{
    kDebug(4640) << "KIconEditGrid::load";
    ...
    *img = image->convertDepth(32);
    ...
    for(int y = 0; y < numRows(); y++)
    {
        ...
        //kDebug(4640) << "Row: " << y;
        kapp->processEvents(QEventLoop::AllEvents, 200);
    }
...
{
    kDebug(4640) << "KIconEditGrid::getImage";
    *image = *img;
}
...
#warning check if this is still necessary
  ...
  QByteArray type;
  foreach (const QByteArray &format, fileFormats )
  {
    type = "image/" + format.toLower();
...
QImage KIconEditGrid::getSelection(bool cut)
{
    ...
    rect.getRect(&nx, &ny, &nw, &nh);

    QImage tmp(nw, nh, QImage::Format_ARGB32);
    clearImage(&tmp);
    ...
    if(ok)
    {
        ...
        || (tmp.size().height() > img->size().height()) )
        {
            if(KMessageBox::warningYesNo(this,
                ...
                " image!\nPaste as new image?"),QString::null,KGuiItem(i18n("Paste")), KGuiItem(i18n("Do Not Paste"))) == 0)
            {
                editPasteAsNew();
            }
            return;
        ...
        {
            //kDebug(4640) << "KIconEditGrid: Pasting at: " << insrect.x() << " x " << insrect.y();
            QApplication::setOverrideCursor(Qt::WaitCursor);
...
{
    kDebug(4640) << "KIconGrid::editResize";
    KResizeDialog *rs = new KResizeDialog(this, QSize(numCols(), numRows()));
    if(rs->exec())
    {
...
{
    kDebug(4640) << "::setSize: " << s.width() << " x " << s.height();
...
void KIconEditGrid::createCursors()
{
  ...
  cursor_normal = QCursor(Qt::ArrowCursor);

  pix = BarIcon("colorpicker-cursor");
  if(pix.isNull())
  {
    cursor_colorpicker = cursor_normal;
    kDebug(4640) << "KIconEditGrid: Error loading colorpicker-cursor.xpm";
  }
  else
  {
  ...
  if(pix.isNull())
  {
    ...
    kDebug(4640) << "KIconEditGrid: Error loading paintbrush.xpm";
  }
  else
  {
    mask = pix.createHeuristicMask();
  ...
  if(pix.isNull())
  {
    ...
    kDebug(4640) << "KIconEditGrid: Error loading fill-cursor.xpm";
  }
  else
  {
    mask = pix.createHeuristicMask();
  ...
  if(pix.isNull())
  {
    ...
    kDebug(4640) << "KIconEditGrid: Error loading aim-cursor.xpm";
  }
  else
  {
    mask = pix.createHeuristicMask();
  ...
  if(pix.isNull())
  {
    ...
    kDebug(4640) << "KIconEditGrid: Error loading airbrush-cursor.xpm";
  }
  else
  {
  ...
  if(pix.isNull())
  {
    ...
    kDebug(4640) << "KIconEditGrid: Error loading eraser-cursor.xpm";
  }
  else
  {
    mask = pix.createHeuristicMask(true);
...
static void QPA_makeEllipse(Q3PointArray& ar, int x, int y, int w, int h )
...
void KIconEditGrid::drawPointArray(Q3PointArray a, DrawAction action)
{
	...
	// added to the history! TODO: Fix this
        macro = new K3MacroCommand( i18n("Drawn Array") );
...
static bool kdither_32_to_8( const QImage *src, QImage *dst )
{
    ...
    if ( !dst->create(src->width(), src->height(), 8, 256) ) {
		kWarning() << "OImage: destination image not valid";
		return false;
	}
